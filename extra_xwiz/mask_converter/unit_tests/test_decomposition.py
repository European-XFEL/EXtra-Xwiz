"""
Tests of the decomposition algorithms.
...
def test_delta_method():
    ...
    A[1:2, 1:3] = True
    B = np.copy(A)
    assert dc.delta_method(A) == [((1, 3), (1, 2))], "Simple rectangle test."
    ...
    assert dc.delta_method(C) == exp_res_C, "Complex shape test."
    D = np.zeros((5, 5), dtype=bool)
    D[0:1, 0:1] = D[2:3, 2:3] = True
    exp_res_D = [((0, 1), (0, 1)), ((2, 3), (2, 3))]
    assert dc.delta_method(D) == exp_res_D, "Test on separate pixels."
